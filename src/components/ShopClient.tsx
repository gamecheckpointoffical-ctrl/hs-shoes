  ...
  return (
    <div className="container-lux pt-16 pb-32">
      <div className="text-center mb-16 pt-8">
        ...
        <h1 className="font-display text-4xl md:text-[48px] mb-4 text-ink">{title}</h1>
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-1 gap-y-2 mb-16">
        <Link href="/shop" className={`text-[10px] uppercase tracking-[0.15em] px-5 py-2.5 border transition-all duration-300 ${
          !currentFilter && !currentCategory ? 'border-ink bg-ink text-cream' : 'border-stone/60 hover:border-ink text-ash hover:text-ink'
        }`}>All</Link>
        <Link href="/shop?filter=new" className={`text-[10px] uppercase tracking-[0.15em] px-5 py-2.5 border transition-all duration-300 ${
          currentFilter === 'new' ? 'border-ink bg-ink text-cream' : 'border-stone/60 hover:border-ink text-ash hover:text-ink'
        }`}>New Arrivals</Link>
        <Link href="/shop?filter=bestseller" className={`text-[10px] uppercase tracking-[0.15em] px-5 py-2.5 border transition-all duration-300 ${
          currentFilter === 'bestseller' ? 'border-ink bg-ink text-cream' : 'border-stone/60 hover:border-ink text-ash hover:text-ink'
        }`}>Bestsellers</Link>
        {categories.map((cat) => (
          <Link key={cat.id} href={`/shop?category=${cat.slug}`} className={`text-[10px] uppercase tracking-[0.15em] px-5 py-2.5 border transition-all duration-300 ${
            currentCategory === cat.slug ? 'border-ink bg-ink text-cream' : 'border-stone/60 hover:border-ink text-ash hover:text-ink'
          }`}>{cat.name}</Link>
        ))}
      </div>
      ...
      {products.length === 0 ? (
        <div className="text-center py-32">
          ...
          <Link href="/shop" className="text-[10px] uppercase tracking-[0.15em] link-underline text-ink">View all products</Link>
        </div>
      ) : (
