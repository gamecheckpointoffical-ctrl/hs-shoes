  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
        scrolled ? 'bg-ink/95 backdrop-blur-xl border-b border-stone/60' : 'bg-transparent'
      }`}>
        <div className="container-lux flex items-center justify-between h-[60px] md:h-[72px]">
          ...
          <nav className="hidden md:flex items-center gap-12">
            <Link href="/shop" className="text-[10px] uppercase tracking-[0.2em] link-underline text-cream/80 hover:text-cream">Shop</Link>
            <Link href="/shop?filter=new" className="text-[10px] uppercase tracking-[0.2em] link-underline text-cream/80 hover:text-cream">New Arrivals</Link>
            <Link href="/shop?filter=bestseller" className="text-[10px] uppercase tracking-[0.2em] link-underline text-cream/80 hover:text-cream">Bestsellers</Link>
            <Link href="/about" className="text-[10px] uppercase tracking-[0.2em] link-underline text-cream/80 hover:text-cream">About</Link>
          </nav>

          <Link href="/" className="absolute left-1/2 -translate-x-1/2">
            <span className="font-display text-[22px] tracking-tight text-cream">HS Shoes</span>
          </Link>

          <div className="flex items-center gap-7 text-cream">
            ...
            <button onClick={() => setIsOpen(true)} className="relative" aria-label="Cart" data-cursor="cart">
              ...
              {itemCount > 0 && <span className="absolute -top-2 -right-2 bg-gold text-ink text-[9px] w-[16px] h-[16px] flex items-center justify-center rounded-full font-medium">{itemCount}</span>}
            </button>
          </div>
        </div>
      </header>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[60] md:hidden">
          <div className="absolute inset-0 bg-ink/60 backdrop-blur-md animate-fade-in" onClick={() => setMobileOpen(false)} />
          <div className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-ink-soft p-12 animate-slide-in flex flex-col">
            <div className="flex items-center justify-between mb-20">
              <span className="font-display text-xl text-cream">HS Shoes</span>
            ...
            <nav className="flex flex-col gap-8">
              {[
                ...
                <Link key={href} href={href} onClick={() => setMobileOpen(false)} className="text-sm uppercase tracking-[0.15em] text-cream/80">{label}</Link>
              ))}
            </nav>
