...
export default function ProductCard({ product, image }: { product: Product; image?: string }) {
  ...
  return (
    <Link href={`/shop/${product.slug}`} className="product-card group block">
      <div className="img-hover aspect-[3/4] bg-stone mb-6 relative overflow-hidden">
        {image && (
      ...
      <div className="space-y-1.5">
        <h3 className="text-[15px] font-medium tracking-tight">{product.name}</h3>
        <p className="text-xs text-ash leading-relaxed">{product.short_description || product.brand}</p>
        <div className="flex items-baseline gap-3 pt-1">
          <span className="text-[15px] font-medium">{formatPrice(product.price, product.currency)}</span>
